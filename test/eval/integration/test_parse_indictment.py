...
#
"""Integration tests for indictment (起诉意见书) parsing.
...
class TestParseIndictment:
    """Test suite for indictment document parsing."""

    def test_parse_single_indictment(
        self,
        integration_setup: EvaluationSetup,
        indictment_single_dataset: str,
    ):
        """Test parsing a single indictment document.

        Uses prebuilt dataset with one pre-parsed indictment document.
        ...
        url = f"{integration_setup.base_url}/api/v1/datasets/{indictment_single_dataset}/documents"
    ...
    def test_parse_multiple_indictments(
        self,
        integration_setup: EvaluationSetup,
        indictment_multiple_dataset: str,
    ):
        """Test parsing multiple indictment documents.

        Uses prebuilt dataset with multiple pre-parsed indictment documents.
        Verifies all documents are in DONE state.

        Args:
            integration_setup: Logged-in EvaluationSetup instance
            indictment_multiple_dataset: Prebuilt dataset ID with multiple indictments
        """
        # Get document list from prebuilt dataset
        session = integration_setup.session
        url = f"{integration_setup.base_url}/api/v1/datasets/{indictment_multiple_dataset}/documents"
        resp = session.get(url)
        data = resp.json()
    ...
    def test_indictment_chunk_count(
        self,
        integration_setup: EvaluationSetup,
        indictment_single_dataset: str,
    ):
        """Test that parsed indictment has generated chunks.
    ...
    def test_indictment_chunk_content(
        self,
        integration_setup: EvaluationSetup,
        indictment_single_dataset: str,
    ):
        """Test that indictment chunks have actual content.
