...
"""
Indictment Opinion Chunker - Uses IndictmentPlugin for section-based parsing.
...
def chunk(filename, binary=None, from_page=0, to_page=100000, lang="Chinese", callback=None, **kwargs):
    """
    Parse indictment opinion PDFs using the two-layer architecture.

    Layer A: Extract UniversalBlocks from PDF using PaddleOCR
    Layer B: Transform blocks to chunks using IndictmentPlugin
    ...
    # Only support PDF for indictment opinions
    if not re.search(r"\.pdf$", filename, re.IGNORECASE):
        callback(-1, "Indictment parser only supports PDF files.")
        return res

    callback(0.1, "Start to parse indictment opinion.")
    ...
    # Layer B: Route to IndictmentPlugin
    ...
    logger.info(f"Indictment chunking complete: {len(res)} chunks from {filename}")
