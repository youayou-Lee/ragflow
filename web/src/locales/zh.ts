...
例如：一条带有 1024 维嵌入的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      reparseType: '重新解析类型',
      fullReparse: '全部重新解析',
      fullReparseDesc: '重新进行 OCR 识别并解析',
      textOnlyReparse: '文本重新解析',
      textOnlyReparseDesc: '使用 OCR 缓存，仅重新解析文本',
...
<i>不符合上述规则的文本行将被忽略。</i>
`,
      interrogation: `<p>仅支持 <b>PDF</b> 文件格式。</p><p>
      本解析器专为中文讯问笔录/询问笔录设计。
      </p><p>
      自动识别以下内容：
      <ul>
        <li><b>头部信息块</b> - 基础信息（时间、地点、讯问人、被讯问人信息等）</li>
        <li><b>问答对块</b> - 笔录中的问与答内容</li>
      </ul>
      </p><p>
...
示例：general/v2/`,
      ...
      fallback: {
        ...
        button: '备用',
        models: '备用模型（同厂商）',
        modelsPlaceholder: '选择同厂商内可回退的模型',
        modelsDesc: '当触发速率限制时，优先尝试同厂商的这些模型',
        factories: '备用厂商（跨厂商）',
        factoriesPlaceholder: '选择其他可回退的厂商',
